...
public class InvoiceConsumer {
    ...
    public void receiveS3Event(TextMessage textMessage) throws JMSException, IOException {
        ...
        log.info("Received S3 Event: {}", messageText);

        SnsMessage snsMessage = objectMapper.readValue(messageText, SnsMessage.class);
        ...
        if (s3EventNotification == null) {
            log.error("S3EventNotification is null");
            return;
        }

        List<S3EventNotification.S3EventNotificationRecord> records = s3EventNotification.getRecords();
        if (records == null || records.isEmpty()) {
            log.error("No records found in S3EventNotification");
            return;
        }

        for (S3EventNotification.S3EventNotificationRecord record : records) {
            if ("s3:TestEvent".equals(record.getEventName())) {
                log.info("Ignoring test event: {}", record.getEventName());
                continue;
            }
    ...
    private void processInvoiceNotification(S3EventNotification s3EventNotification) throws IOException {
        ...
        for (S3EventNotification.S3EventNotificationRecord s3EventNotificationRecord : records) {
            ...
            if (s3Entity == null || s3Entity.getBucket() == null || s3Entity.getObject() == null) {
                log.error("Invalid S3Entity in the record: {}", s3EventNotificationRecord);
                continue;
            }
